...
class MongoDBManager:
    ...
    def __init__(
        ...
        mongo_init_db: Optional[str] = None,
    ...
    async def connect(self):
        ...
        if self.is_testing:
            ...
            password = quote_plus(
                "jr8ZjCE2myTvXr1OH8ZBFT4EBk2c7wkqu"
            )  # заэнкодит спецсимволы
            uri = f"mongodb://{username}:{password}@127.0.0.1:27017/db?authSource=db"
            logger.info(self.mongodb_uri)
            self._client = AsyncIOMotorClient(uri)  # self.mongodb_uri
